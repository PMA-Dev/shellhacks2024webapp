...
export const pushLog = async (data: Log): Promise<number> => {
    const db = await getDbHandle();
    await db.read();
    data.id = getRandomInt(10000000, 100000000);
    await db.update((dbData: DbData) => dbData.logs.push(data));
    return data.id;
};
...
export const editLogInPlace = async (
    ...
    await db.update((dbData: DbData) => {
        const obj = dbData.logs.find((x: Log) => x.id == parentQueryId);
        if (!obj) {
        ...
        fnToAppendId(dbData.logs.find((x: Log) => x.id == parentQueryId));
...
export const query = async (queryId: number): Promise<Log | null> => {
    ...
    if (!fetchedData) {
        ...
        throw Error('No data found for query id:' + queryId);
    }
    return fetchedData;
};
